...
def calculate_dqn_loss(model, target_model, experiences, gamma,
    ...
    if isinstance(next_states, tuple):
        ...
        next_q_values = model(next_states).squeeze(1)
    action_unsqueezed = actions.long().unsqueeze(1)
    curr_q_value = q_values.gather(1, action_unsqueezed)

    # compute target values
    if isinstance(next_states, tuple):
        next_target_q_values = target_model(*next_states).squeeze(1)
    else:
        next_target_q_values = target_model(next_states).squeeze(1)
...
def calculate_soft_dqn_loss(model, target_model, experiences, gamma,
                            alpha=0.1, use_double_q_update=False, reward_clip=None, reward_scale=None):
